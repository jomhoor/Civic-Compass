...
function proj(p: { x: number; y: number; z: number }, cx: number, cy: number, s: number) {
...
export default async function TwitterImage() {
  ...
  const axisEnds = axes3D.map((a) => {
    ...
    return { ...proj({ x: r.x * AXIS_OVERSHOOT, y: r.y * AXIS_OVERSHOOT, z: r.z * AXIS_OVERSHOOT }, cx, cy, S), z: r.z };
  });

  const dataEnds = axes3D.map((a, i) => {
    const v = AXES[i].value;
    const r = rot(a, angleY, angleX);
    return { ...proj({ x: r.x * v, y: r.y * v, z: r.z * v }, cx, cy, S), z: r.z };
  });

  const gridSteps = [0.33, 0.66, 1.0];
  const gridRings = gridSteps.map((gv) =>
    axes3D.map((a) => {
      const r = rot({ x: a.x * gv, y: a.y * gv, z: a.z * gv }, angleY, angleX);
  ...
  const triangles = axes3D.map((_, i) => {
    ...
    return { points: `${cx},${cy} ${dataEnds[i].x},${dataEnds[i].y} ${dataEnds[i2].x},${dataEnds[i2].y}`, fill: hexRGBA(AXES[i].color, alpha) };
  });

  const depthOrder = axes3D.map((_, i) => ({ i, z: dataEnds[i].z })).sort((a, b) => a.z - b.z);

  const labelEnds = axes3D.map((a) => {
    const r = rot(a, angleY, angleX);
    const lk = AXIS_OVERSHOOT * 1.35;
    return { ...proj({ x: r.x * lk, y: r.y * lk, z: r.z * lk }, cx, cy, S), z: r.z };
  });
    ...
    (
      <div style={{ width: "100%", height: "100%", display: "flex", background: "linear-gradient(135deg, #0a0a0f 0%, #121228 50%, #0d1117 100%)", fontFamily: "Parastoo, system-ui, sans-serif", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", width: "840px", height: "100%", position: "relative" }}>
          <svg width="840" height="630" viewBox="0 0 840 630" style={{ position: "absolute" }}>
            {gridRings.map((ring, idx) => (
              <polygon key={`grid-${idx}`} points={ring} fill="none" stroke="rgba(255,255,255,0.1)" strokeWidth="1" />
            ))}
            {AXES.map((a, i) => (
              <line key={`axis-${i}`} x1={cx} y1={cy} x2={axisEnds[i].x} y2={axisEnds[i].y} stroke={a.color} strokeWidth="1.2" opacity="0.4" />
            ))}
            {AXES.map((a, i) => (
              <circle key={`axdot-${i}`} cx={axisEnds[i].x} cy={axisEnds[i].y} r="3" fill={hexRGBA(a.color, 0.7)} />
            ))}
            {triangles.map((t, i) => (
              <polygon key={`tri-${i}`} points={t.points} fill={t.fill} />
            ))}
            <polygon points={dataPolygon} fill="rgba(91,157,245,0.15)" stroke="rgba(91,157,245,0.75)" strokeWidth="2" />
            {depthOrder.map(({ i: idx, z }) => {
              ...
              return (
                <g key={`node-${idx}`}>
                  <circle cx={dp.x} cy={dp.y} r={nodeR + 4} fill={hexRGBA(AXES[idx].color, alpha * 0.25)} />
                  <circle cx={dp.x} cy={dp.y} r={nodeR} fill={hexRGBA(AXES[idx].color, alpha)} />
                </g>
              );
            })}
            <circle cx={cx} cy={cy} r="2" fill="rgba(200,220,255,0.25)" />
          </svg>
          ...
          {depthOrder.map(({ i: lIdx, z: lz }) => {
            ...
            return (
              <div key={`label-${lIdx}`} style={{ position: "absolute", left: `${le.x - 60}px`, top: `${le.y - 10}px`, width: "120px", textAlign: "center", color: hexRGBA(AXES[lIdx].color, alpha), fontSize: "13px", fontWeight: 600, display: "flex", justifyContent: "center" }}>
                {AXES[lIdx].key}
              </div>
            );
          })}
        ...
        <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", paddingRight: "60px", width: "360px", gap: "16px" }}>
          <div style={{ fontSize: "40px", fontWeight: 700, color: "#ffffff", lineHeight: 1.15, display: "flex", flexDirection: "column" }}>
            <span>Civic</span>
            <span>Compass</span>
          </div>
          <div style={{ width: "48px", height: "3px", background: "linear-gradient(90deg, #5B9DF5, #A78BFA)", borderRadius: "2px", display: "flex" }} />
          <div style={{ fontSize: "16px", color: "rgba(255, 255, 255, 0.6)", lineHeight: 1.5, display: "flex", flexDirection: "column" }}>
            <span>Your politics has a shape,</span>
            <span>not a side.</span>
          </div>
          <div style={{ fontSize: "12px", color: "rgba(255, 255, 255, 0.3)", letterSpacing: "0.15em", textTransform: "uppercase", display: "flex" }}>
            compass.jomhoor.org
          </div>
        </div>

        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: "3px", background: "linear-gradient(90deg, #0EBB90, #8CDAF5, #FEEB34, #E87461, #A78BFA, #34D399, #F59E0B, #60A5FA)", display: "flex" }} />
      </div>
    ),
    {
      ...size,
